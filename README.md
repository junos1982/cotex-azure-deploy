...
## 구성 파일
- `main.tf`: `azurerm` 프로바이더를 선언하고 Azure Resource Graph 쿼리를 사용해 구독 내 리소스 그룹을 조회합니다.
...
1. GitHub 리포지토리의 **Settings > Secrets and variables > Actions** 메뉴에서 다음 시크릿을 생성합니다.
   ...
   - (선택) `AZURE_CLIENT_SECRET` – 서비스 프린시플 비밀을 사용하는 경우에만 필요합니다. [Federated credentials](https://learn.microsoft.com/azure/developer/github/connect-from-azure?tabs=azure-portal%2Clinux#use-an-azure-ad-application-with-federated-credentials) 를 사용하는 경우에는 설정하지 않습니다.
2. 워크플로를 수동으로 실행하려면 GitHub 리포지토리의 **Actions** 탭에서 `List Azure Resource Groups` 워크플로를 선택하고 **Run workflow**를 클릭합니다.
3. 워크플로는 `main` 브랜치에 푸시될 때마다 자동으로 실행되며, Terraform을 통해 리소스 그룹 목록을 조회한 뒤 `resource_group_names`와 `resource_groups_raw` 출력을 표시합니다. 워크플로는 Azure 로그인 후 Terraform에 필요한 `ARM_*` 환경 변수를 자동으로 설정하므로 별도의 수동 설정이 필요하지 않습니다.
